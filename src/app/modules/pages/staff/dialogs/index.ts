import { StaffActionComponent } from './staff.action/staff.action.component';
import { StaffCreateUserComponent } from './staff.create.user/staff.create.user.component';
import { StaffResetPasswordComponent } from './staff.reset.password/staff.reset.password.component';
import { StudentActionComponent } from './student.action/student.action.component'
import { IntershipAcitonComponent } from './intership.aciton/intership.aciton.component'
import { EmployeeCourseActionComponent } from './employee-course.action/employee-course.action.component'
import { StaffUploadComponent } from './staff.upload/staff.upload.component'
import { StudentUploadComponent } from './student.upload/student.upload.component'
import { IntershipUploadComponent } from './intership.upload/intership.upload.component'

export { StaffActionComponent } from './staff.action/staff.action.component';
export { StaffCreateUserComponent } from './staff.create.user/staff.create.user.component';
export { StaffResetPasswordComponent } from './staff.reset.password/staff.reset.password.component';
export { StudentActionComponent } from './student.action/student.action.component'
export { IntershipAcitonComponent } from './intership.aciton/intership.aciton.component'
export { EmployeeCourseActionComponent } from './employee-course.action/employee-course.action.component'
export { StaffUploadComponent } from './staff.upload/staff.upload.component'
export { StudentUploadComponent } from './student.upload/student.upload.component'
export { IntershipUploadComponent } from './intership.upload/intership.upload.component'

export const DIALOGS = [
  StaffActionComponent,
  StaffCreateUserComponent,
  StaffResetPasswordComponent,
  StudentActionComponent,
  IntershipAcitonComponent,
  EmployeeCourseActionComponent,
  StaffUploadComponent,
  StudentUploadComponent,
  IntershipUploadComponent
];
